...
!
      REAL(KIND=DPSIZE) :: AMOUNT,DSUM,G2,GG,METIME,MINTIM,MSTART,OLDA,RATE,T1,T2,TMIN
      DATA TMIN /-100.0D+00/
!      
...
!        
        IF (SECOND .AND. (ADVID == 6 .OR. ADVID == 8 .OR. ADVID==13 .OR. ADVID==14 .OR. SSID == 6)) THEN   
...
!
 2100 CONTINUE !7.2
      ...
      DELTA=T2-T1
!
      IF (ADVID == 9 .OR. ADVID==15) THEN      ! CALLA, CALLE, CALLP initialized by PREDI at ICALL=1
...
! Bauer.  SADVAN2->SADVAN under normal circumstances.  EM algorithms may redirect for efficiency purposes 
      ...
      ELSE      ! Allow SS=3 (Like 1, but use existing amounts as initial estimates)
        IF ((ISSNOW == 3 .OR. EVTREC(NEVENT,JSS) == 3) .AND. &
          (SSID == 6 .OR. SSID == 9 .OR. SSID==13 .OR. SSID==14 .OR. SSID==15)) THEN
          SS3=1
