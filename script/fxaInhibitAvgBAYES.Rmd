...
title: |
       ...
       |
       | Hands-on 1: METHOD = BAYES example
...
\log\left(\gamma\right) &\sim N\left(1, 1\right)_+ \\
...
geninit <- function(){
    ...
    paste("(0, ", rnorm(1, 2, 0.25), ")   ; GAMMA = THETA(3)"),
    paste(rnorm(1, log(5.9), 0.25), "  ; SD = EXP(THETA(4)) "),
...
nChains <- 4
nPost <- 10000 ## Number of post-burn-in samples per chain after thinning
nBurn <- 10000 ## Number of burn-in samples per chain after thinning
nThin <- 1
...
if(fitModel){
  ...
  runChains <- mclapply(1:nChains, runChain,
                      ...
                      seed = seed, print = 100,
                      OLKJDF = 0, OVARF = 1, AUTO = 1,
                      method = "BAYES", grid = TRUE, mode = "nonpara")
  }
