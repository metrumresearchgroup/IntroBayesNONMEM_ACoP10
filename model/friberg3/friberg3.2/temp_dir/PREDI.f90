...
! Allow no basic params with DES
          IF ((NBP <= 0 .OR. NBP >= IRGG) .AND. (NBP /= 0 .OR.      &
              (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 9 .AND. ADVID /= 13.AND.ADVID/=14 .AND. ADVID/=15))) THEN   ! ajb 3/2009              
            IF (NBP <= 0) THEN
...
! Check for error in compartment definitions; Allow no dose compartment with ADVAN9
          IF ((IDC <= 0 .OR. IDC > NCM1) .AND. (IDC /= 0 .OR. (ADVID /= 9 .AND. ADVID/=15) )) THEN
...
!        
        ...
        ELSE
          MQQ=SD
        ENDIF
        IF ( (ADVID /= 9 .OR. ADVID /=15) .OR. .NOT. OLDFMT) THEN
          IF(MQQ<=8) THEN 
...
!        
        DO I=1,NC
          ...
          ELSE
            WRITE (LINE,220) I,(IATT(I,J),J=6,7),SPACE(1:MQQ-3),COFFON(ISV(I)+1),CNOYES(ITURN(I)+1),  &
                             CNOYES(IINST(I)+1),CNOYES(JD),CNOYES(JDO),COTHER(1),COTHER(2)
          END IF
          IF ( (ADVID == 9 .OR. ADVID==15) .AND. OLDFMT) THEN
            MLINE=MQQ+88
          ELSE
            MLINE=MQQ+66
          END IF
        ...
        END DO
!        
        IF ( (ADVID == 9 .OR. ADVID==15) .AND. .NOT. OLDFMT) THEN
          IF(MQQ<=8) THEN
        ...
        END IF
! Special checks for ADVAN9
        IF ( (ADVID == 9 .OR. ADVID==15) .AND. LNCM2 > 0) THEN 
          DO I=LNCM1+1,NCM1
            IF (IINST(I) == 1) THEN
...
! 
        IF ((SSID /= 0 .AND. SSID /= 6 .AND. SSID /= ADVID) .AND.  &
            (SSID /= 5 .OR. (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 13 .AND. ADVID /= 14 )) .AND.   &
            (SSID /= 9 .OR. ( ADVID /=15)) .AND.   &
            (SSID /= 7 .OR. (ADVID /= 6 .AND. ADVID /= 8 .AND. ADVID /= 13 .AND. ADVID /= 14 ))) THEN       
...
!
      IF (ADVID == 6 .OR. ADVID == 8 .OR. ADVID == 9 .OR. ADVID == 13 .OR. ADVID==14 .OR. ADVID==15) THEN       ! ajb 3/2009      
...
! Local Variables
!
      INTEGER(KIND=ISIZE) :: ICALL,ICALLDP,ICALLP
!
      IF(TOLTOPRED==1) THEN
        IF(ADVID/=6.AND.ADVID/=8.AND.ADVID/=9.AND.ADVID/=10.AND.ADVID/=13.AND.ADVID/=14.AND.ADVID/=15) GO TO 999
      ENDIF
      ICALLP=ICALL
      ICALLDP=ICALLD
      ICALL=0
